...
Below is an example of a FLIM experiment (performed by Dr. Ajit Joglekar) from the [reference study](https://github.com/CreLox/FluorescenceLifetime#how-to-cite-this-work) where a high FRET efficiency was observed. Here in the experimental group (red), the nuclear pore complex protein NUP50 is tandemly tagged by mNeonGreen (the donor fluorophore) and mScarlet-I (the acceptor fluorophore). In the control group (black), NUP50 is only tagged by mNeonGreen. We can see that the lifetime of mNeonGreen in NUP50-mNeonGreen-mScarlet-I is greatly shortened compared to the lifetime of mNeonGreen in NUP50-mNeonGreen. By fitting the FLIM data with (multi-component) exponential decays, we can then quantify the FRET efficiency using the equation above.
