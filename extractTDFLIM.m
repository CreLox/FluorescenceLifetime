% [FlimData, Image] = extractTDFLIM(PhotonCountFilter, ExcludePixelIndices)
function [FlimData, Image] = extractTDFLIM(PhotonCountFilter, ...
    ExcludePixelIndices)
...
function [FlimData, Image] = parseFlimData(TDFLIMFilePath, ...
    ...
    for ChannelNo = 1 : FlimProp.ChannelCount
        ...
        for x = 1 : FlimProp.FramePixelWidth
            for y = 1 : FlimProp.FramePixelHeight                    
                FlimData(ChannelNo).PixelDecayHistogram{x, y} = ...
                    fread(FileID, FlimProp.ADCResolution, 'uint16');
                Image(ChannelNo, x, y) = ...
                    sum(FlimData(ChannelNo).PixelDecayHistogram{x, y});
                if (Image(ChannelNo, x, y) <= max(PhotonCountFilter)) && ...
                        (Image(ChannelNo, x, y) >= min(PhotonCountFilter)) && ...
                        ~ismember((x - 1) * FlimProp.FramePixelHeight + y, ...
                        ExcludePixelIndices)
                    FlimData(ChannelNo).AllPixelsDecayHistogram = ...
                        FlimData(ChannelNo).AllPixelsDecayHistogram + ...
                        FlimData(ChannelNo).PixelDecayHistogram{x, y};
